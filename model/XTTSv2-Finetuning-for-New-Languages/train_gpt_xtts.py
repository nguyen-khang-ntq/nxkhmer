...
def train_gpt(metadatas, num_epochs, batch_size, grad_acumm, output_path, max_audio_length, max_text_length, lr, weight_decay, save_step):
    ...
    # Set languages AFTER loading config to include Khmer
    config.languages = ["km"]
    ...
    # Override character limit for km language to match max_text_length
    if hasattr(config, 'characters_config') and config.characters_config:
        if 'km' in config.characters_config:
            config.characters_config['km']['characters_limit'] = max_text_length
    ...
    # Get local rank for GPU assignment (set by torchrun)
